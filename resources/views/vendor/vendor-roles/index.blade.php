                                        ...
                                        <td>
                                            @if($role->name <> 'admin')
                                                @if(Auth::guard('vendor')->user()->hasRole('admin') || Auth::guard('vendor')->user()->can('vendor-vendor-roles-update'))
                                                    <a href="{{ route('vendor.vendor-roles.edit',$role->id) }}"
                                                ...
                                                @if(Auth::guard('vendor')->user()->hasRole('admin') || Auth::guard('vendor')->user()->can('vendor-vendor-roles-delete'))
                                                    <button class="btn btn-default btn-sm" title="Delete"
                                                       onclick="if (confirm(&quot;Are you sure you want to delete ?&quot;)) { document.getElementById('deleteForm{{ $role->id }}').submit(); } event.returnValue = false; return false;" {{ in_array($role->id,$vendor_assigned_role) ? 'disabled' : '' }}><i
                                                            class="fa fa-trash"></i></button>

                                                    {!! Form::open(['method'=>'DELETE', 'action'=>['vendor\VendorRoleController@destroy', $role->id], 'id'=>'deleteForm'.$role->id]) !!}
                                                    {!! Form::close() !!}
                                                @endif
                                            @endif
                                        </td>
