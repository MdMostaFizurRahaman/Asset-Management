            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="box">
                        ...
                        {!! Form::open(['method'=>'GET', 'action'=>['client\AssetController@index', $subdomain]]) !!}
                        ...
                        <div class="box-body table-responsive ">
                            <table class="table table-bordered table-hover">
                                <tr>
                                    ...
                                    <th class="" style="min-width:50px;">Move Actions</th>
                                ...
                                @foreach($assets as $asset)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            {!! Helper::assetAssignType($asset->assign_user) !!}
                                        </td>
                                        ...
                                        <td>{{ $asset->attachments ? $asset->attachments->count() : '0' }}</td>
                                        ...
                                        <td>
                                            @if(Auth::user()->hasRole('admin') || Auth::user()->can('client-asset-return-approval-create'))
                                                @if($asset->accept_reject_status==3)
                                                    <a href="#" class="btn btn-default btn-sm" title="Accept"
                                                       onclick="if (confirm(&quot;Are you sure you want to accept return ?&quot;)) { document.getElementById('acceptForm{{ $asset->id }}').submit(); } event.returnValue = false; return false;"><i
                                                            class="fa fa-check"></i></a>
                                                    {!! Form::open(['method'=>'POST', 'action'=>['client\AssetController@returnaccept', $subdomain, $asset->id], 'id'=>'acceptForm'.$asset->id,'style'=>'display:none']) !!}
                                                    {!! Form::close() !!}

                                                    <a href="{{ route('client.return.reject', [$subdomain, $asset->id]) }}"
                                            ...
                                            @if($asset->assign_user == 0 && $asset->accept_reject_status == 0)
                                                @if(Auth::user()->hasRole('admin') || Auth::user()->can('client-assets-update'))
                                                    <a title="Change Store" href="{{ route('client.assets.move.order', [$subdomain, $asset->id]) }}"
                                        ...
                                        <td>
                                            <a title="Details" href="{{ route('client.assets.show', [$subdomain, $asset->id]) }}"
                                               class="btn btn-default btn-sm"><i class="fa fa-eye"></i></a>
                                            @if(Auth::user()->hasRole('admin') || Auth::user()->can('client-assets-update'))
                                                <a title="Edit" href="{{ route('client.assets.edit', [$subdomain, $asset->id]) }}"
                                                   class="btn btn-default btn-sm"><i class="fa fa-edit"></i></a>

                                                <a title="Attach file" href="{{ route('client.assets.attach.file', [$subdomain, $asset->id]) }}"
                                                   class="btn btn-default btn-sm"><i class="fa fa-file"></i></a>
                                            @endif
                                            @if($asset->assign_user ==0)
                                                @if(Auth::user()->hasRole('admin') || Auth::user()->can('client-assets-update'))
                                                    <a title="Assign User" href="{{ route('client.assets.assignuser', [$subdomain, $asset->id]) }}"
                                                       class="btn btn-default btn-sm"><i
                                            ...
                                            @if(Auth::user()->hasRole('admin') || Auth::user()->can('client-assets-delete'))
                                                <a href="#" class="btn btn-default btn-sm" title="Delete"
                                                   onclick="if (confirm(&quot;Are you sure you want to delete ?&quot;)) { document.getElementById('deleteForm{{ $asset->id }}').submit(); } event.returnValue = false; return false;"><i
                                                        class="fa fa-trash"></i></a>

                                                {!! Form::open(['method'=>'DELETE', 'action'=>['client\AssetController@destroy', $subdomain, $asset->id], 'id'=>'deleteForm'.$asset->id]) !!}
                                                {!! Form::close() !!}
                                            @endif

                                            @if(Auth::user()->hasRole('admin') || Auth::user()->can('client-assets-show'))
                                                <a title="Asset Logs" href="{{ route('client.assets.logs', [$subdomain, $asset->id]) }}"
                                                   class="btn btn-default btn-sm"><i
                                                        class="fa fa-book"></i></a>
                                            @endif

                                            @if(Auth::user()->hasRole('admin') || Auth::user()->can('client-asset-permission-create'))
                                                <a title="Vendor Permission" href="{{ route('client.assets.vendor.permission', [$subdomain, $asset->id]) }}"
                                                   class="btn btn-default btn-sm"><i
